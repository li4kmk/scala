Конвертация данных из Excel-файла в JSON-формат. Читаются данные из двух листов Excel (тарелки и штрудели), обрабатываются, формируются структурированные запросы и сохраняется результат в JSON-файл.

1. Инициализация и загрузка Excel-файла
Скрипт начинает с вывода текущей рабочей директории.
Загружает Excel-файл по указанному пути (fileSource).
Открывает два листа: тарелки и штрудели.
